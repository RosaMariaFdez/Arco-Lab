...
int main(int argc, char const *argv[])
{
    ...
    sycl::buffer<int> filtrox{weights,R}, filtroy{weights_y,R}, altura{img_width}, anchura{img_height};
    int rango = img_height*img_width*2;
    ...
    Q.submit([&](sycl::handler &h){
        sycl::accessor accX{filtrox, h};
        sycl::accessor accY{filtroy, h};
        sycl::accessor accAL{altura, h, sycl::read_only};
        sycl::accessor accAN{anchura, h, sycl::read_only};

        h.parallel_for(rango, [=](sycl::id<2> i){
            double sum_x = 0.0;
            double sum_y = 0.0;
            ...
            for(int i = -1; i <= 1; i++) {
                for(int j = -1; j <= 1; j++) {
                    ...
                    int original_value = gray_scale_image[(item[0]+i) * img_width + sycl::item[1] + j];
                    sum_x += weight * original_value;
                    sum_y += weight_y * original_value;
                }
            }
            int G = unsigned(std::sqrt(sum_x*sum_x+sum_y*sum_y));
            output_image[sycl::item[0] * img_width + sycl::item [1]] = std::min(G, 255);});     
    });
